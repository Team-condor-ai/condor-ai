...
// Plantilla ajustada el 28-ago-2026 a partir de 3 referencias reales que
// pasó Joaquín (WhatsApp Image 2026-08-28) para el carrusel de "servicios /
// agentes IA". Gradiente y hex tomados del set ya aprobado (mismo azul/
// violeta/carmesí que usan los íconos de T_NOTICIAS), no inventados.
const T_SERVICIOS = `Centred agency statement slide, 4:5. Background near-white #F7F7F8. One vivid diagonal gradient bloom (blue #1B4DE4 → violet #7B2FBF → crimson #E8203A) blurred softly into ONE corner only (top-left OR bottom-right — pick one per slide, never both, never centred) — clearly visible, not a faint tint. ${ZONA_LOGO_CENTRO} EVERYTHING IS CENTRE-ALIGNED on one vertical axis. One simple thin-line icon (line-art only, never filled) centred above the headline, stroked in that same blue-violet-crimson gradient — pick whichever icon best fits this slide's point (a clock, a chat bubble, a speedometer, a checkmark, a bar chart) and never more than one icon per slide. Very large headline in heavy grotesque sans, near-black #0A0A0A, tight leading, two or three short centred lines. Optionally, NOT on every slide, one or two key words inside the headline are filled with that same gradient instead of black, for emphasis only — most slides can be fully black. Under the headline, two or three centred lines of medium-grey #666666 supporting copy at a noticeably smaller size. At the very bottom, a short centred horizontal rule in the same gradient — that rule is the LAST element in the frame, nothing else follows it below. Enormous breathing room, symmetric margins, poster-like calm. NO photos, NO people, NO clip-art, NO cards, NO boxes, NO drop shadows, and NO second icon, circle, ring or shape anywhere besides the one icon above the headline — a bare outlined circle floating below the rule is a mistake, not a design element.`;
